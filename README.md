...
下の図は、2人で入室し、双方がIPv6対応、LANケーブルで有線接続の環境で、音の途切れや遅延がなく割とうまくいった場合の例です。「遅延／ロス率」が上段下段（送受信）とも「27ms／0%」、「バッファサイズ」が「3ms」となっていることに注目してください。（遅延／ロス率は、上段↑が送信、下段↓が受信）  
お手元で試された時に、近い数値になったでしょうか。
...
4. セッションする予定の人たちと、できれば事前に１対１で接続テストする。事前テストが無理でも、セッション演奏を始めるまえに、全参加者のひとりひとりと、個々にバッファサイズの調整をする時間を設けた方がよい。「音の途切れ」と「遅延のなさ」はトレードオフなので、バランスをみる。
5. 入室したら、「メンバー設定エリア」を「詳細表示」にし、遅延とバッファサイズを確認する。理想としては、遅延が送受信とも30ms以下ぐらい。バッファサイズを小さくすればするほど遅延も小さくなるというわけでもなく、遅延が一番小さくなるように、スライダーでバッファサイズを微調整する。  
...
|05|バッファサイズ|64|実際の接続時には自動調整されるので、64に固定でよい。音が途切れる場合、ここで調節するのではなく、メイン画面の途切れる相手のスライダーで調節する。|
|06|入力のモニタリング|モニタリングする（遅延あり）|オーディオインターフェイスのダイレクトモニタリング機能は切る。|
|07|基本音質設定|高音質|帯域は使うが遅延にはさほど影響ない。原則は「高音質」でよい。|
|08|その他の設定|「オーディオデータ補間処理を有効にする」をオン|参加者全員の音が途切れるという場合のみ、オフも試してみるが、原則はオン。|
...
1. 相手が入室すると、「メンバー設定エリア」に相手が表示される。自分が主催の場合、相手のアイコンの左側に、「黒丸バツ」ボタンが表示されるが、これを押すと相手を強制退室させられる。
